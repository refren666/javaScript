    ...
    if (error) {
        ...
        brushTeeth(true, (error2, brush) => {
            if (error2) {
                ...
                console.log(brush);
                eatFood(true, (error3, eat) => {
                    if (error3) {
                        console.error(error3);
                    } else {
                        console.log(eat);
                        dressUp(true, (error4, dressing) => {
                            if (error4) {
                                console.error(error4);
                            } else {
                                console.log(dressing);
                                commuting(true, false, (error5, commute) => {
                                    if (error5) {
                                        ...
                                        doWork(work => {
                                            console.log(work);
                                            goBackHome(true, (error6, goBack) => {
                                                if (error6) {
                                                    console.error(error6);
                                                } else {
                                                    console.log(goBack);
                                                    eatFood(true, (error7, eat) => {
                                                        if (error7) {
                                                            console.error(error7);
                                                        } else {
                                                            console.log(eat);
                                                            brushTeeth(true, (error8, brush) => {
                                                                if (error8) {
                                                                    console.error(error8);
                                                                } else {
                                                                    console.log(brush);
                                                                    goSleep(true, (error9, finish) => {
                                                                        if (error9) {
...
//     try {
//         const start = await startOfADay(false); // start = result from resolve/reject in startOfADay
//         console.log(start);
//         const brush = await brushTeeth(start); // brush = result from resolve/reject in brushTeeth...
//         console.log(brush);
